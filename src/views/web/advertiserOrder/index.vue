            ...
            <div class="content_l shadow">
                ...
                <main>
                    ...
                    <div class="pay-list" v-show="type==1">
                        <p class="alipy">
                            {{orderDateils.payment == 2 ? '选择微信' :(orderDateils.payment == 3 ? '选择支付宝' : '选择银行卡') }}
                        </p>
                        <CollectionTemplate :data="data" class="payItem" />
                        <div class="more">
                            <img src="@/assets/transactions/ico_add_gray.png" />
                            <span
                                @click="bindPay">新增{{ orderDateils.payment  == 1 ? "银行卡" : orderDateils.payment  == 2 ? "微信" : "支付宝" }}收款方式</span>
                        </div>
                    </div>

                    <!--底部按钮-->
                    <button class="btn-publish" v-if="type == 1" @click="handleSubmit">发布卖出挂单</button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 新增支付方式
            bindPay() {
                const type = this.orderDateils.payment
                if (type == 1) {
