<template>
  <div class="register">
    ...
    <div>
      ...
      <button class="button" :disabled="disabled" :class="{active:!disabled}" @click="Submitted">注册</button>
      <router-link to="/mlogin" class="btn1">已有账号，去登录</router-link>
    </div>
  </div>
</template>
      ...
      if (this.pwd1 !== this.pwd2) {
        ...
        buyApi
          ...
          .then(res => {
            ...
            if (res.kyc <= 0) {
              this.$router.push('/mregistrationSuccess')
            } else {
